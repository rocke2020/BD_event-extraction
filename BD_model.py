...
class Net(nn.Module):
    ...
    def predict_triggers(self, tokens_x, mask, head_indexes, arguments=None, Test=False):
        ...
        ### adjusts bert out sequence location by head_indexes before go to RNN
        for i in range(batch_size):
            x[i] = torch.index_select(x[i], 0, head_indexes[i])
        
        trigger_logits = self.fc_trigger(x)
        # shape: batch_size, seq-len
        triggers_predicted = trigger_logits.argmax(-1)

        x_rnn, h0, predicted_triggers_with_batch_num = [], [], []
        for i in range(batch_size):
            predicted_triggers = find_triggers([idx2trigger[trigger] for trigger in triggers_predicted[i].tolist()])
            for predicted_trigger in predicted_triggers:
                ...
                x_rnn.append(x[i])
                predicted_triggers_with_batch_num.append((i, t_start, t_end, t_type_str))

        argument_logits, arguments_true_match_predicted = [0], [0]
        arguments_predicted = [{'events': {}} for _ in range(batch_size)]
        if len(predicted_triggers_with_batch_num) > 0:
            ...
            if Test == False:
                # shape: (N_trigger L N_argument), N is the number of predicted triggers
                for batch_num, t_start, t_end, t_type_str in predicted_triggers_with_batch_num:
                    a_label = [NONE] * x.shape[1]
                    if (t_start, t_end, t_type_str) in arguments[batch_num]['events']:
                        for (a_start, a_end, a_role) in arguments[batch_num]['events'][(t_start, t_end, t_type_str)]:
        ...
        return trigger_logits, triggers_predicted, argument_logits, arguments_true_match_predicted, arguments_predicted
